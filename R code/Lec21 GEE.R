...
resp_gee3 <- gee(nstat ~ centre + treatment + gender + baseline + age, data = resp, family = "binomial", id = subject,
                 corstr = "unstructured", scale.fix = TRUE, scale.value = 1) # scale parameter is phi (over dispersion)
...
head(epilepsy)
itp <- interaction(epilepsy$treatment, epilepsy$period) # define the interaction term 
tapply(epilepsy$seizure.rate, itp, mean) # crude check of group mean by period*treatment

# fit log linear GEE
per <- rep(log(2),nrow(epilepsy)) # Create a log2: as offset 
# 2 week per period, so log2 is the offset (for seisure rate per week)
...
epilepsy_gee4 <- gee(seizure.rate ~ offset(per) + base + age + period*trt, data = epilepsy, family = "poisson", id = subject, corstr = "exchangeable", scale.fix = FALSE)
