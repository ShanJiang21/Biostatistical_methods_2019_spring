...
m = rowSums(car[,1:3])
## plug in m in the equation 
res.pearson = (car[,1:3] - pihat*m)/sqrt(pihat*m) # pearson residuals 
res.pearson


## WARNING: residuals in R built-in should not be used directly. 
...
## No. of groups = 6; categories = 3;
## In total, we have 2 models, 2 * 4 = 8 parameters;

# deviance   
D.stat =  sum(2 * car[,1:3] * log(car[,1:3]/(m * pihat)))
D.stat

## Twice the log likelihood of full model - fitted model


# Continue with inference: Fit a new model  
# alternative model is beta_age3 = 2*beta_age2

## come up with a new covariate: age.new 
...
## 2\beta_{22}X_3: new covairate remains a same parameter 
