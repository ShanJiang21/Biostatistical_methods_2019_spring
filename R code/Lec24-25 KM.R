...
########################################################
param1 <- flexsurvreg(Surv(time, cens) ~ 1, 
...
plot(KM, conf.int = FALSE, mark.time = TRUE,
     ...
     main="6MP K-M curve", cex.lab=1.5, cex.main=1.5)

plot(KM)

# cumulative hazard function 
plot(KM,fun='cumhaz', title =  "cumulative hazard function") 

# estimate cumulative hazard rates
cbind(KM$time,-log(KM$surv),  cumsum(KM$n.event/KM$n.risk))
# time, KM est, Nelson-Aalen Estimator


# obtain survival rate at given time, with CI
summary(KM,time=c(5,10,12.5, 15)) 
# note: n.event is the cumulative num of events since last listed time 
# get any time points of the survival rate 

summary(KM, censored = TRUE)
# (if not specify time, then n.event is the # event at each time point)


# median survival time --> 50% s(t), with CI
print(KM)
